...
| 2.4 | Price observations (submit, pending/approved/rejected) | PARTIAL | PARTIAL | MISSING | COMPLETE | PARTIAL | — | `services/price_service.py`, `price_history` table, `test_price_panel_rule`; composer captures `price_paid`; no moderation surface for observations |
| 2.5 | **Price History panel** (≥3 independent observations) | COMPLETE | MISSING | MISSING | COMPLETE | MISSING | — | `price_service.py` implements the ≥3-distinct-submitter rule; no frontend panel anywhere |
...
| 2.8 | Duplicate detection / existing-product matching | MISSING | MISSING | MISSING | MISSING | MISSING | — | no dedup path found on the submission route |
