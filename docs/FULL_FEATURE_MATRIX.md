...
| 4.13 | Action Menu "Rate a Seller" state | N/A | COMPLETE | — | COMPLETE | MISSING | — | Enabled → `/sellers/rate` now that 4.4–4.8 exist; `action-menu-model.test.mjs`. Reachable from the mounted mobile menu (X.11); `/sellers/rate` added to the proxy guard so sign-in returns to it |
...
| 8.3 | L3 Reverse image search + metadata | BLOCKED | — | PARTIAL | COMPLETE | BLOCKED | **EXTERNAL_SERVICE_REQUIRED** — provider unnamed in the PRD (`[AMBIGUOUS]`) and none procured. Built as 8.2: `ImageCheckProvider`, empty `IMAGE_ADAPTERS`, `REVERSE_IMAGE_PROVIDER`, `reverse_image_status` = `not_configured`; the queue card's tile reads "No provider" and fails closed on any unknown status, and the frame's RIS table column draws a dash for an unrun check — never a tick, which would read as a clean result nobody produced. Photo metadata inspection is not built | PRD FR-8; same evidence as 8.2 |
...
| 8.6 | L6 Community reporting + escalation | COMPLETE | COMPLETE | COMPLETE | COMPLETE | MISSING | — | Off-production (0049 `report_resolution`): a report can now be answered, which it could not before — `POST /admin/reports/{id}/decision` closes it as dismissed / content_removed / content_restored / escalated, performs the content action through the same `referral_service` the review routes use, and writes both the outcome on the report row and an audit entry for the moderator. The queue defaults to OPEN reports (`?status=resolved` for the rest); the duplicate guard now means "one OPEN report", so a reader whose report was answered may report a repeat offence. Console: `ReportDecision.tsx` on the Report tab. `test_report_resolution_api` (9), `report-resolution-labels.test.mjs`; DB tests in CI |
...
| 8.10 | Flagged voters, High/Low trust segmentation, risk levels | MISSING | MISSING | MISSING | — | MISSING | **NO DATA AND NO METHODOLOGY** | Audited 2026-09-16 against the implementation, which the row previously said was outstanding. There is nothing to connect: votes carry no geography, no trust bucketing and no fraud classification, so there is no flagged-voter aggregate, no High/Low trust split and no per-voter risk level anywhere in the backend. Frame 6922:837 draws all of it plus a five-level risk table; the console states the absence in place (`FLAGGED_VOTERS_UNAVAILABLE`) rather than drawing a plausible number, which on a fraud screen is the one mistake worth designing against. Contract §18 names the five levels; defining what earns each one is a product decision, not engineering work |
...
| 9.2 | Queue states In Review / Archive | PARTIAL | PARTIAL | PARTIAL | PARTIAL | PROVISIONAL | — | The frame's two segments are drawn. "In review" is the live queue. **Nothing archives a review**: there is no archived state in the schema — a decided review is published, rejected or unpublished — so the second segment has no list behind it and says so. Closing this properly needs an archive concept in the contract, not a tab in the console |
...
| 9.4 | Queue item data (product, author, trust, receipt status, age) | COMPLETE | COMPLETE | COMPLETE | COMPLETE | MISSING | — | Audited against frame 6922:837 on 2026-09-16 and rebuilt to its eight columns in its order: Product, Author, Level (the author's trust stage), Wilson, Receipt (`has_receipt`), RIS (8.3), Priority, Age. Every column reads a field the API serves; the backend's `review_id`, which the frame has no column for, rides in the row tooltip. `ReviewQueueScreen.tsx`, `review-queue-model.test.mjs` |
| 9.5 | Reviewer snapshot (account age, trust score, review counts) | COMPLETE | COMPLETE | COMPLETE | COMPLETE | MISSING | — | Audited 2026-09-16. All four figures the frame draws now come off the queue card: Age, Trust Score, Total Reviews and — newly connected — Verified Reviews. `users.verified_review_count` always existed and `QueueAuthor` simply did not carry it, so the console drew it as unavailable rather than borrowing `author_review_count`, which counts unverified work too. A card from an older API still reads unavailable rather than zero. Contract §17; `test_referral_api`, `review-queue-model.test.mjs` |
| 9.6 | Engagement signals (upvotes, views, shares, reports, comments) | PARTIAL | PARTIAL | PARTIAL | COMPLETE | MISSING | Views are **BLOCKED BY DESIGN**; shares have no source | Audited 2026-09-16, field by field. Upvotes and downvotes: served, drawn. Reports: served from the report feed, and absence from a partial feed reads as unavailable rather than zero. Comments: newly connected — a grouped count per page, removed comments excluded, the same number the public cards have carried since BUG-006. **Views: deliberately not served.** `review_view_buckets` exists, but `test_telemetry_isolation` requires every module feeding a moderation decision to stay independent of reading telemetry, so the buckets stay on the reviewer's own dashboard. **Shares: nothing in the backend counts them** — no column, no event table, no endpoint. Top comment: comments come back oldest-first with no ranking, so there is no server-side top one to show. Each absent cell states its reason in place |
| 9.7 | Inline report surfacing (category, description, history) | COMPLETE | COMPLETE | COMPLETE | COMPLETE | MISSING | — | Off-production. The Report tab shows each open report's reason, the reporter's note, the target's title and publish state, how many separate reports that target has attracted, a link into the item, and the four decisions (8.6). A resolved report shows its outcome in words instead of controls. contract §19; `test_reports_api`, `test_report_resolution_api` |
| 9.8 | Approve/reject with star-rating routing | COMPLETE | COMPLETE | COMPLETE | COMPLETE | MISSING | — | Off-production. The backend routing was always there (`test_affiliate_transitions`); until 2026-09-16 no screen could fire it. `ReviewDecision.tsx` in the queue's detail pane: Publish (armed before it fires), Monetize & publish (link + platform + sub-id), Reject (reason required, sent to the author verbatim). The panel states the earn-eligibility outcome before the press — 2★ or lower routes to the Honesty Fund, higher is approved. **BUSINESS-REQUIRED / DESIGN-SYSTEM ALIGNED**: frame 6922:837 draws no controls, and the owner ruled on 2026-09-16 that inspection-only is not acceptable |
| 9.9 | Penalties, suspension, vote-weight suspension | COMPLETE | COMPLETE | COMPLETE | COMPLETE | PROVISIONAL | — | `routes/admin_users.py`, `test_admin_user_management`. Role changes are now the root owner's alone — see X.12 |
| 9.10 | Audit log (filterable) | COMPLETE | PARTIAL | PARTIAL | PARTIAL | PROVISIONAL | Filtering is by action only | `moderate/activity` reads `GET /admin/activity` over `moderation_logs`, which is the single audit surface for content decisions, role changes, receipt views, imports and payouts. Every decision added on 2026-09-16 writes to it with its actor. The gap is the console's filtering, not the data: the log carries actor, target, action, reason, notes and a JSON context, and the screen offers action alone |
...
| X.3 | **3D / 360 product experience** | N/A | N/A | N/A | N/A | N/A | — | **DEFERRED — ASSET / CONTENT NOT AVAILABLE FOR THIS RELEASE** (owner clarification, 2026-09-16). Not in the PRD; contract §20 asked for it and the owner has since taken it out of this release. No asset metadata and no 3D assets exist, so nothing is built and no placeholder viewer or fake rotation control is drawn — a control that spins nothing would claim a capability the product does not have |
...
| X.11 | **Mobile action menu — entry to all three composers** | N/A | COMPLETE | — | COMPLETE | MISSING | — | Owner direction 2026-09-14 (required for QA). Mounted, phones only (hidden ≥ 768px), on `/`, `/feed`, `/search`, `/categories`, `/questions`, `/reviews/[id]`, `/sellers/[id]`; never on the composers. Ask a Question → `/questions/new`, Rate a Seller → `/sellers/rate` (find store → rate → write → done), Write a Review → `/reviews/new`; all three guarded by `proxy.ts` so sign-in returns to the workflow. Visual source: owner frames + canvas screenshot (FIGMA_SOURCE_ACCESS_BLOCKED) — FAB, discs, close and label pills match the frame at 390 and 393 to the pixel; star glyph colour is an **INTENTIONAL PRODUCT DIFFERENCE**. `e2e/action-menu.spec.ts` (14), `route-guards.spec.ts`, `action-menu-model.test.mjs`. Signed-in workflow completion needs a session and is not automated. Branch only |
| X.12 | **Root-owner rule for staff roles** | COMPLETE | COMPLETE | COMPLETE | COMPLETE | MISSING | — | Owner requirement, 2026-09-16. Only `settings.root_owner_email` (bluntly.ph@gmail.com) may grant or revoke moderator/administrator: `is_root_owner` requires the configured email AND the `is_super_admin` column, neither of which is a role, so no role grant can manufacture the authority. `require_root_owner` guards both role endpoints server-side; `can_manage_roles` tells the console whether to offer the controls, and hiding them is never the boundary. Every change is audited with actor, target, from, to and timestamp. A second super admin is refused with `root_owner_required`. Verified against production before the guard was written: the root owner resolves to one existing account, so nobody is locked out. `security.py`, `admin_users.py`, `test_admin_user_management` (5 new) |
| X.13 | **Half-star and zero ratings** | COMPLETE | COMPLETE | — | COMPLETE | MISSING | — | Owner requirement, 2026-09-16. 0, 0.5, 1 … 5 end to end: 0048 makes the three star columns `numeric(2,1)` with a CHECK that pins the half step; the schemas validate it; `StarRatingInput` is an 11-radio group with half and whole hit areas per star and an explicit "No stars (0)" option. **Zero is an intentional rating, not a missing answer** — the composer gate now tests `rating !== null`, not truthiness. The seller card's two graded dimensions stay whole-number chips, as the frame draws them. `test_half_star_ratings` (31), `star-ladder.test.mjs`, `composer-gate-model.test.mjs` |
| X.14 | **Profile Comments and Stats sections** | COMPLETE | COMPLETE | — | COMPLETE | MISSING | — | Owner requirement P0.3, 2026-09-16; frames 5446:6398 and 5446:6532. New endpoint `GET /users/{id}/comments` (a member's own comments, newest first, with the parent review — removed comments and comments on unpublished or removed reviews never appear). Stats reads `GET /users/{id}/trust`, which now serves `progress`: the next stage, its name and the reviews needed, computed from the trust ladder itself so the frontend carries no second copy of the thresholds. The frame's "Frequent Interactions" card is **not drawn** — there is no follow graph, no interaction count and no endpoint, so it would be three invented faces. All three sections are one route switched by `?tab=`. `test_profile_comments_api` (7), `test_trust_api`, `profile-tabs-model.test.mjs` |
| X.15 | **Google sign-in, honestly disabled** | N/A | COMPLETE | — | COMPLETE | MISSING | — | Owner requirement P0.2, 2026-09-16. "Continue with Google" is drawn on both auth screens, greyed, `disabled` and `aria-disabled="true"`, with "Coming soon — sign up/log in with your email below." under it. It navigates nowhere, and there is no configuration in which it does: the flag-gated OAuth hand-off was removed once the contract audit showed the backend publishes no OAuth route, so that branch could only ever reach a 404. Re-enabling it needs the backend routes first, then the Google Cloud client id and secret the project owner holds. `account-menu-guards.test.mjs` |
| X.16 | **Production QA seller** | N/A | N/A | N/A | N/A | COMPLETE | — | Owner requirement P0.8, 2026-09-16. One row in production `sellers`: `Bluntly QA Seller`, id `5e11e700-0000-4000-8000-000000000999`, platform `other` (never Shopee or Lazada), `unclaimed`, no store URL, no reviews, no ratings, no revenue. A directory entry so seller journeys can be exercised live, not a merchant identity. Reversible with one DELETE, recorded in `docs/QA_HANDOFF.md` |

## Explicitly not built (design-only prototype rows, contract §30)

| Item | Status | Reason |
|---|---|---|
| Bookmarks | N/A | Not in the PRD or the completion contract; one prototype row is not a product requirement. The account menu lists only real destinations, and `account-menu-guards.test.mjs` fails if this row comes back |
...
- Deferred by the owner rather than missing: 3D/360 (X.3) — see its row.
- **A "MISSING" production column on a row whose other columns read COMPLETE
  means the work is on the branch and has not been deployed yet.** It is not a
  claim that the feature does not exist, and it is not a claim that it works in
  production either. Both are settled by the deployment, not by this file.

## What changed on 2026-09-16 (owner's final verdict)

Rows touched by the P0 fixes and by §30: 8.3, 8.6, 9.2, 9.4, 9.7, 9.8, 9.9,
X.3, and the new X.12 – X.16. Two things in that list are **BUSINESS-REQUIRED /
DESIGN-SYSTEM ALIGNED** rather than Figma-sourced — the review decision panel
and the report decisions — because the approved frames draw a console that can
only inspect, and the owner ruled that inspection alone is not acceptable for
moderation. Both are built from the console's existing panel, pill and
two-step-arming vocabulary, and both are named as differences here and in
`docs/QA_HANDOFF.md` rather than presented as the design.
