...
| X.15 | **Google sign-in, honestly disabled** | N/A | COMPLETE | — | COMPLETE | MISSING | — | Owner requirement P0.2, 2026-09-16. "Continue with Google" is drawn on both auth screens, greyed, `disabled` and `aria-disabled="true"`, with "Coming soon — sign up/log in with your email below." under it. It navigates nowhere: the real OAuth hand-off exists in the component but is reached only when `NEXT_PUBLIC_GOOGLE_AUTH` is "1", which needs a Google Cloud client id and secret the project owner holds. `account-menu-guards.test.mjs` |
