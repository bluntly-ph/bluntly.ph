...
| 2.4 | Price observations (submit, pending/approved/rejected) | COMPLETE | PARTIAL | COMPLETE | PARTIAL | MISSING | — | `a6183cb`, off-production (QA freeze): 0044 status/source/review_id; `/admin/price-observations` + `/moderate/prices`; self-decision 422, second decision 409. "Let's talk money" now files a pending observation when a marketplace is picked (`price-capture-model.test.mjs`). Frontend PARTIAL: no standalone "report a price" form outside the composer. DB tests written, run in CI |
