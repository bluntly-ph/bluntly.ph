...
## ⚠️ TEMPORARY — read before testing (valid only until M3 is pushed)

**Status as of 2026-07-16:** `origin/main` is **M2** (`cfca10b`, migrations up to
`0009_tokens`), but **both databases — local AND Supabase — have already been
migrated to `0014_schema_parity`** (the M3 schema), because M3 was built and
verified against them before it was pushed. This is a **code-vs-database** gap,
not a local-vs-prod one.

### What still works — everything this plan tests ✅

The M3 schema is **additive**: extra tables M2 never queries, and every new
NOT NULL column carries a server default. **Verified 2026-07-16** by running the
pushed M2 code (`cfca10b`) against the live Supabase `0014` database — **7/7
flows passed**: review submission · attach link + publish · attribution redirect
· voting + Wilson · trust profile · token economy · moderator queue + fraud
signals.

So the **§0b path in the M1 plan works unchanged**:
```bash
USE_SUPABASE=true python -m scripts.db_check        # CONNECTED ok, 25 public tables
USE_SUPABASE=true uvicorn app.main:app --port 8001  # then run every group below
```
`db_check` reporting **25** tables instead of 19 is expected here — those are the
M3 tables. Not a failure.

### What breaks — the Alembic step only ❌

With M2 code checked out, **any Alembic command against either database fails**:

```
alembic upgrade head   ->  ERROR: Can't locate revision identified by '0014_schema_parity'
```

The revision file only exists in the M3 commits. Consequences while M3 is unpushed:

| Action | Result |
|---|---|
| `USE_SUPABASE=true uvicorn app.main:app` (the plan's path) | ✅ works — **use this** |
...
**Automatically, once M3 is pushed** — no cleanup, no data change. The M3 commits
carry migrations `0010`–`0014`, so the code will contain the revision the
databases are stamped with, and Alembic + docker-compose line up again. Until
then this note stands; delete it when M3 lands.

*(The alternative — rolling both databases back to `0009` — is possible but
pointless: it would discard the verified M3 schema to make a temporary window
tidier.)*
