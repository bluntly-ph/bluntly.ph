        ...
        {result.ok ? (
          ...
          <div className="mt-6">
            ...
            <p className="mt-2 text-[14px] text-[var(--text-secondary)]">
              Only the store&rsquo;s approved owner can see this. If you run this store, claim it
              from its page; a moderator checks every claim before anything changes.
            </p>
          </div>
        ) : (
...
function Dashboard({ dashboard }: { dashboard: SellerDashboard }) {
  ...
  return (
    <>
      ...
      <h1 className="text-[22px] font-bold leading-snug text-[var(--text-primary)]">
        {seller.display_name}
      </h1>
      ...
      <section aria-labelledby="waiting-heading" className="mt-10">
        ...
        {waiting.length === 0 ? (
          <p className="mt-4 text-[14px] text-[var(--text-secondary)]">Nothing is waiting on the store.</p>
        ) : (
            ...
            {waiting.map((q) => (
              <li key={q.id} className="border-b border-[var(--line-hairline-10)]">
                <Link href={`/questions/${q.id}`} className="flex items-start gap-3 py-4 no-underline">
                  <ChatCircle size={18} aria-hidden="true" className="mt-0.5 shrink-0 text-[var(--accent-primary)]" />
                  <span className="min-w-0">
                    <span className="block text-[14px] font-medium text-[var(--text-primary)]">{q.body}</span>
